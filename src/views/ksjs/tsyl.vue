				...
				<el-row :gutter="20">
					<el-col :md="4">
						<el-image v-if="item.img_url" style="width: 100%; height: auto; cursor: pointer;"
							:src="item.img_url" :key="item.img_url" @click="go_detail('/kstsyl-detail?id=' + item.id)">
						</el-image>
					</el-col>
					<el-col :md="20">
						<div style="line-height: 50px; margin-left: 40px;">
							<div>
								<el-link :underline="false" :href="'/kstsyl-detail?id=' + item.id" target="_blank">
									{{ item.title }}
								</el-link>
							</div>
...
<script>
	...
	export default {
		...
		setup() {
			...
			const get_data = () => {
				axios.get('/api/head/offices/tsyl', {
					params: {
						id: id,
					...
					state.office_name = res.data[0] ? res.data[0].office_name : ''
				})
			}
