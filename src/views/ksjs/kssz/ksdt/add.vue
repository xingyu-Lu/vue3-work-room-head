<template>
	...
	<el-breadcrumb separator="/" style="margin-bottom: 20px;">
		...
		<el-breadcrumb-item>部门动态</el-breadcrumb-item>
	</el-breadcrumb>
	...
	export default {
		...
		setup() {
			...
			onMounted(() => {
				...
				if (id) {
					axios.get(`/api/head/technicalOfficeDynamics/${id}`).then(res => {
						state.Form = {
			...
			const submitAdd = () => {
				Ref.value.validate((vaild) => {
					if (vaild) {
						...
						let url = '/api/head/technicalOfficeDynamics'
						if (id) {
							// params.id = id
							// 修改商品使用 put 方法
							params.release_time = state.Form.release_time
							httpOption = axios.put
							url = `/api/head/technicalOfficeDynamics/${id}`
						}
