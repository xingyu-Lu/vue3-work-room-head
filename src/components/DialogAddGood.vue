<template>
	<el-dialog :title="type == 'add' ? '添加商品' : '修改商品'" v-model="visible">
...
<script>
	...
	export default {
		...
		setup(props) {
			...
			// 获取详情
			const getDetail = (id) => {
				axios.get(`/good_info.php?id=${id}`).then(res => {
					state.ruleForm = {
						name: res.goods.goodsName,
						id: res.goods.goodsId,
						link: res.goods.goodsCoverImg,
						sort: res.goods.stockNum
					}
				})
			}
			// 开启弹窗
